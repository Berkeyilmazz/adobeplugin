...
from the default `1.00` to **`0.99`**, and saves every result as **PNG**.
...
3. Processed copies are written to a `processed/` subfolder as `.png` files —
...
- Every processed file is saved as `.png` (e.g. `photo.jpg` → `processed/photo.png`).
